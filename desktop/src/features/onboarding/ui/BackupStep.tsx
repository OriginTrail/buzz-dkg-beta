...
const isDkgBetaBuild =
  import.meta.env?.VITE_BUZZ_DKG_BETA?.toLowerCase() === "true";
...
export function BackupStep({
  ...
  const storageDescription =
    identityStorage === "system-keyring"
      ...
      : identityStorage === "local-file"
        ? isDkgBetaBuild
          ? "This beta deliberately avoids Keychain prompts. Your identity key is stored in an owner-only file on this Mac and is not encrypted at rest. Export a separate backup before relying on this identity."
  ...
  const introStorageDescription =
    identityStorage === "system-keyring"
      ...
      : identityStorage === "local-file"
        ? isDkgBetaBuild
          ? "This beta stores your identity key in an owner-only local file and does not access macOS Keychain."
