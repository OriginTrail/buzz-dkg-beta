import { BrainCircuit, Database, RefreshCw, Sparkles } from "lucide-react";
import { Button } from "@/shared/ui/button";
import { Card } from "@/shared/ui/card";
import type { DiscoveryEntry, MemoryGate } from "../api";
...
export function MemoryProvisioningGate({
  enabling,
  error,
  onStart,
}: {
...
export function MemoryFallback({
  ...
  entries: DiscoveryEntry[] | undefined;
  loading: boolean;
  gate: MemoryGate;
  cg: string | null;
}) {
  return (
    <>
        ...
        <div className="flex items-start gap-2">
          ...
          <div>
            ...
            <p className="mt-0.5 text-2xs leading-relaxed text-muted-foreground">
              Receipt-based records are shown below for discovery. They are not
              independently verified by this app.
            </p>
          </div>
        </div>
      </Card>
      ...
      <GateNotice gate={gate} cg={cg} />
    </>
  );
}

function GateNotice({ gate, cg }: { gate: MemoryGate; cg: string | null }) {
