...
#[test]
fn keyringless_build_persists_and_reloads_owner_only_identity_file() {
    let dir = tempfile::tempdir().unwrap();
    let key_path = dir.path().join("identity.key");

    let first = load_or_create_identity(dir.path()).unwrap();
    assert_eq!(first.storage, IdentityStorage::LocalFile);
    assert_eq!(first.recovery, RecoveryState::None);
    ...
    assert_eq!(second.storage, IdentityStorage::LocalFile);
    assert_eq!(
        first.keys.public_key().to_hex(),
        second.keys.public_key().to_hex()
    );
}
