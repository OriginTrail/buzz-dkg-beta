<h1 align="center">Buzz DKG Beta 🐝</h1>

<p align="center">
  <strong>Buzz with shared, provenance-aware memory powered by OriginTrail DKG.</strong>
</p>

<p align="center">
  An isolated beta distribution for evaluating the Buzz–DKG integration. It installs alongside the regular Buzz app with a separate profile.
</p>
...
The 💭 roadmap item *"Web-of-trust reputation across relays"* has running code in this branch. Reputation here is not a score — it is the **traceable record of a channel's decisions and the reasoning behind them**, surfaced as a first-class panel and resolved through an [OriginTrail DKG](https://origintrail.io) provider.

Click the floating **◈ Memory** chip in any channel with `@dkg` activity. The panel prefers a node *you* run, falls back to the authenticated community DKG provider, and uses clearly unverified receipt discovery only when neither is available.
...
<tr>
<td width="50%" valign="top">
  <img src="docs/assets/screenshots/dkg-memory-verified.png" alt="The memory panel: three memory layers, the latest decision, and per-participant topic sub-graphs, verified through the viewer's own node" width="100%">
  <sub><b>What this channel remembers.</b> Three memory layers (Working / Shared / Verifiable), the latest decisions, and per-participant sub-graphs — 200 decisions across 10 participants, <b>✓ verified through your own node</b>.</sub>
</td>
<td width="50%" valign="top">
  <img src="docs/assets/screenshots/dkg-memory-evidence.png" alt="A decision expanded to show its provenance: built from three source messages, checked by the viewer's node" width="100%">
  <sub><b>Why, not just what.</b> Expand any decision to see its provenance — here it is <b>built from 3 source messages</b>, each traceable, so you can reconstruct the reasoning behind a conclusion.</sub>
</td>
</tr>
</table>

**Reputation here is not a score** — it is the traceable record of a channel's decisions and the reasoning behind them:

- **Three memory layers** — Working / Shared Working / Verifiable Memory, so nothing becomes anchored (or costly) by accident.
- **Provenance, not just verdicts** — every decision links back to the events it derives from (the evidence trail above).
- **Per-participant sub-graphs** — reconstruct *why* a given human or agent concluded what they did.
- **Local-first, with honest fallbacks** — memory resolves through your own node (🟢), the community DKG provider (🔵), or, when neither is reachable, a relay-receipt **discovery mode** clearly labeled *unverified* (🟡).

Nostr expresses the trust (signed events); the DKG preserves and connects it (provenance, shared memory, optional anchored identifiers) — always **Nostr-first**. See **[docs/dkg-memory.md](docs/dkg-memory.md)** for the full write-up and design rationale, and **[docs/rfc-web-of-trust.md](docs/rfc-web-of-trust.md)** for the RFC this prototype implements.

<sub>Experimental, feature-flagged, and advisory-only — it never gates identity, membership, moderation, or writes.</sub>
